...
import { useSnapshot, timeAgo, shortId, fmtDuration } from "@/lib/useSnapshot";
...
export default function VisualPage() {
  const { snapshot, connected, now } = useSnapshot(1000);
      ...
      {sessions.length > 0 && selected && (
        <>
          ...
          <div className="office-panel">
            ...
            <OfficeScene
              mainLabel={selected.agentName ?? "team-lead"}
              mainSub={runningTime(
                selected.active,
                selected.startedAt,
              ...
              desks={desks}
            />
            <SessionTrace project={selected.project} sessionId={selected.id} />
          </div>
        </>
      )}
    </main>
